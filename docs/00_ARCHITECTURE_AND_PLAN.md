...
> 1. **No separate backend.** All orchestration (compiler, indexer/NAV, keepers, taker bot) is a server-only module of the Next app in THIS repo (server actions + `pnpm aqua:*` scripts), persisting to Postgres via Drizzle. See SRV rules in `01_BUSINESS_RULES.md` and section 4.5.
> 2. **This dedicated repo replaces pool-party-frontend branches.** Contracts (Foundry), the Next app (server module + demo UI), scripts, and these canonical docs all live here. Issue branches `<type>/aqua-poo-<num>-<slug>` off `main`, PR per issue.
> 3. **Section 4.6 (frontend) is re-scoped for the hackathon window:** investor/manager surfaces are built as this repo's demo UI (see the re-scope comments on POO-1064/1067/1068); integration into the main pool-party-frontend app behind the `aquaStrategies` flag is post-hackathon (epic D6).
> 4. Work item map + coordination protocol for parallel sessions: `02_WORKPLAN.md`.
